...
const NavbarVertical = (props) => {
  ...
  const generateLink = (item) => {
    if (item.link === "/pages/editarUsuarioActual?" && userInfo) {
      item.link = item.link + "reference=" + userInfo[0].reference;
    }

    if (item.link === "/pages/perfilUsuarioActual?" && userInfo) {
      // console.log(userInfo)
      const redirectToProfile = () => {
        const prop = { correo: userInfo[0].correoElectronico };
      ...
      <SimpleBar style={{ maxHeight: "100vh" }}>
        <div className="nav-scroller">
          <Link href="/" className="navbar-brand">
            <Image src="/images/brand/logo/tripnary_logo.png" alt="" />
          </Link>
        </div>
        {/* Dashboard Menu */}
        <Accordion
